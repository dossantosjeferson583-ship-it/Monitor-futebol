...
def buscar_resultado_ht_fixture(fixture_id):
    """
    Consulta uma partida e retorna o placar do intervalo quando disponível.
    """
    jogos = api_get("fixtures", {"id": fixture_id})
    if not jogos:
        return None
    ...
    # Só valida depois que o primeiro tempo realmente terminou.
    if status not in {"HT", "2H", "FT", "AET", "PEN"}:
        return None

    ht = jogo.get("score", {}).get("halftime", {})
    gols_casa_ht = ht.get("home")
    gols_fora_ht = ht.get("away")

    if gols_casa_ht is None or gols_fora_ht is None:
        return None
    ...
    return {
        "fixture_id": fixture_id,
        "status": status,
        "casa": jogo.get("teams", {}).get("home", {}).get("name", "Mandante"),
        "fora": jogo.get("teams", {}).get("away", {}).get("name", "Visitante"),
        "ht_casa": int(gols_casa_ht),
        "ht_fora": int(gols_fora_ht),
        "ft_casa": gols.get("home"),
        "ft_fora": gols.get("away"),
    }


def mensagem_resultado_ht(item, resultado):
    """
    Classifica o alerta estatístico HT como confirmado ou não confirmado.
    """
    total_gols_ht = resultado["ht_casa"] + resultado["ht_fora"]
    acertou = total_gols_ht >= 1

    status_texto = (
        "✅ ACERTO — houve pelo menos 1 gol no 1º tempo."
        if acertou
        else "❌ ERRO — o 1º tempo terminou 0x0."
    )

    linhas = [
        "📌 RESULTADO DO ALERTA ESTATÍSTICO",
        "",
        f"⚽ {resultado['casa']} x {resultado['fora']}",
        f"🔥 Índice HT enviado: {item['indice_ht']}/100",
        "",
        f"⏱️ Intervalo: {resultado['ht_casa']} x {resultado['ht_fora']}",
    ]

    if resultado["ft_casa"] is not None and resultado["ft_fora"] is not None:
        linhas.append(
            f"🥅 Placar atual/final: {resultado['ft_casa']} x {resultado['ft_fora']}"
        )
    ...
    linhas.extend([
        ...
        status_texto,
        "",
        "ℹ️ Validação automática do critério estatístico do alerta.",
    ])

    return "\\n".join(linhas)


def verificar_resultados_ht_uma_vez():
    """
    Verifica se os alertas selecionados já chegaram ao intervalo.
    Envia apenas um resultado por partida.
    """
    for item in list(SELECIONADOS_ATUAIS):
        fixture_id = item.get("fixture_id")

        if not fixture_id or fixture_id in RESULTADOS_HT_ENVIADOS:
            continue

        resultado = buscar_resultado_ht_fixture(fixture_id)

        if not resultado:
            continue

        if telegram(mensagem_resultado_ht(item, resultado)):
            RESULTADOS_HT_ENVIADOS.add(fixture_id)
            total_gols_ht = resultado["ht_casa"] + resultado["ht_fora"]
            classificacao = "ACERTO" if total_gols_ht >= 1 else "ERRO"
            log(
                f"📌 Resultado HT enviado: "
                f"{resultado['casa']} x {resultado['fora']} | {classificacao}"
            )
    ...
    while True:
        try:
            ...
            if SELECIONADOS_ATUAIS:
                ...
                verificar_resultados_ht_uma_vez()
